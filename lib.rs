...
impl<G: Group> BbsSignatureParams<G> {
    /// Generates deterministic parameters for the BBS scheme based on the desired number of attributes to sign.
    ///
    /// # Arguments
    ...
    /// # Returns
    /// - `Ok(())`: the MAC is valid
    /// - `Err(BbsSignatureError::InputLengthMismatch)`: number of attributes does not match number of generators
...
impl BbsProof {
    ...
    //                undisclosed_attribute.push(messsages[i]);
    //                ctr_undisclosed+=1;
    //                i+=1;
    //            }
